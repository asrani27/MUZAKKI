...
<form class="form-horizontal" action="{{route('editpenyuluhan', $data->id)}}" method="POST">
            {{ csrf_field() }}
          <div class="box-body">
            <br />
                ...
                <div class="col-sm-10">
                  <select id="peserta" class="form-control select2" style="width: 100%;" name="pegawai_id" required>
                      @foreach ($pegawai as $ag)
                        ...
                        <option value="{{$ag->id}}" selected>{{$ag->nik}} - {{$ag->nama}}</option>
                        @else
                        <option value="{{$ag->id}}">{{$ag->nik}} - {{$ag->nama}}</option>
                        @endif
                      @endforeach
                  </select>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                  <select id="pegawai" class="form-control select2" style="width: 100%;" name="kecamatan_id" required>
                      @foreach ($kecamatan as $ag)
                        @if($data->pegawai_id == $ag->id)
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
              <input type="text" class="form-control" name="tempat_tugas" value="{{$data->tempat_tugas}}">
              </div>
            </div>
