...
<form class="form-horizontal" action="{{route('edittransaksi', $data->id)}}" method="POST">
            {{ csrf_field() }}
          <div class="box-body">
            <br />
            <div class="form-group">
              ...
              <div class="col-sm-10">
              <input type="text" class="form-control" name="no_kw" required value="{{$data->no_kw2}}" readonly>
              </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <div class="input-group date">
                        ...
                        <input type="text" class="form-control pull-right" name="tgl" id="datepicker" value="{{ Carbon\Carbon::parse($data->tgl)->format('m/d/Y') }}">
                    </div>
                </div>
            </div>
                ...
                <div class="col-sm-10">
                  <select id="peserta" class="form-control select2" style="width: 100%;" name="peserta_id" required>
                      @foreach ($peserta as $ag)
                        @if($data->peserta_id == $ag->id)
                        <option value="{{$ag->id}}" selected>{{$ag->nama_peserta}}</option>
                        @else
